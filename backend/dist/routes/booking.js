...
router.post('/create', async (req, res) => {
    try {
        ...
        const existingBooking = await db.get('SELECT id FROM bookings WHERE bus_id = ? AND seat_number = ? AND is_scanned = FALSE', [busId, seatNumber]);
        if (existingBooking) {
        ...
        const result = await db.run('INSERT INTO bookings (user_id, bus_id, seat_number, qr_code, qr_expires_at) VALUES (?, ?, ?, ?, ?)', [userId, busId, seatNumber, qrData, qrExpiresAt.toISOString()]);
        res.json({
...
router.post('/validate', async (req, res) => {
    try {
        ...
        const db = (0, database_1.getDatabase)();
        const booking = await db.get('SELECT * FROM bookings WHERE user_id = ? AND bus_id = ? AND seat_number = ? AND is_scanned = FALSE', [userId, busId, seatNumber]);
        if (!booking) {
        ...
        await db.run('UPDATE bookings SET is_scanned = TRUE WHERE id = ?', [booking.id]);
        await db.run('INSERT INTO trips (bus_id, driver_id, revenue, passengers_boarded) VALUES (?, ?, 1, 1)', [busId, null]);
        res.json({
            message: 'APPROVED',
...
router.get('/driver/:driverId', async (req, res) => {
    try {
        ...
        const db = (0, database_1.getDatabase)();
        const bus = await db.get('SELECT id FROM buses WHERE driver_id = ?', [driverId]);
        if (!bus) {
