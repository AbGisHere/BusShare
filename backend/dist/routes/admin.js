...
router.get('/users', async (req, res) => {
    const db = (0, db_1.getDB)();
    const users = await db.all(`SELECT u.id, u.name, u.phone, u.role, u.created_at, COALESCE(w.balance, 0) AS walletBalance
     FROM users u
...
router.post('/users', async (req, res) => {
    ...
    const result = await db.run('INSERT INTO users (name, phone, role) VALUES (?, ?, ?)', [name, phone, role]);
...
router.get('/buses', async (req, res) => {
    const db = (0, db_1.getDB)();
    const buses = await db.all(`SELECT b.id, b.number, b.is_active, b.current_lat, b.current_lng, b.last_updated,
...
router.post('/buses/:id/assign', async (req, res) => {
    ...
    const bus = await db.get(`SELECT b.*, u.name AS driverName, r.name AS routeName
     FROM buses b
     LEFT JOIN users u ON b.driver_id = u.id
     LEFT JOIN routes r ON b.active_route_id = r.id
     WHERE b.id = ?`, [busId]);
...
router.post('/buses/:id/unassign', async (req, res) => {
    const busId = parseInt(req.params.id, 10);
    const db = (0, db_1.getDB)();
...
router.post('/routes', async (req, res) => {
    ...
    const result = await db.run('INSERT INTO routes (name, stops, description) VALUES (?, ?, ?)', [name, stopsJson, description || null]);
