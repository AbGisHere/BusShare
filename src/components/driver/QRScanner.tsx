    ...
    scanner.start(
      ...
      async (decodedText) => {
        ...
        try {
          ...
          if (ttsEnabled) {
            const msg = res.valid
              ? `Valid ticket. Welcome aboard, ${res.passengerName || 'passenger'}.`
              : `Invalid ticket. ${res.reason || ''}`;
            speak(msg);
          }
        } catch (err: any) {
          setResult({ valid: false, reason: 'Network error. Please try again.' });
...
=======
            <p style={{ fontSize: 12, color: '#52796F', marginTop: 8 }}>
...
=======
                <div style={{
                  position: 'absolute', inset: 0,
                  background: result.valid ? 'rgba(82,183,136,0.92)' : 'rgba(231,111,81,0.92)',
                  display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                  gap: 8, padding: 20,
                }}>
                  <div style={{ fontSize: 52 }}>{result.valid ? '✓' : '✗'}</div>
...
=======
            {state === 'scanning' && (
              <p style={{ fontSize: 12, color: '#52796F', textAlign: 'center', marginBottom: 16 }}>
            ...
            {state === 'result' && (
              <div style={{ display: 'flex', gap: 10 }}>
                <button className="btn btn-outline" style={{ flex: 1 }} onClick={handleScanAnother}>
                  Scan Another
                </button>
